...
export default function Signup() {
  ...
  const handleSubmit = async(e: React.FormEvent) => {
    ...
    if (!settingOtp) {
      ...
      setLoading(true);
      await axios.post(`${BACKEND_URL}/user/signup`, { username: name, email }, { withCredentials: true }).then(() => {
    ...
    <div className="flex-col-12 flex pt-4  sm:pt-0 ">
      ...
      <div className="hidden sm:block w-7/12 ">
        <div className="hidden sm:block w-full h-screen p-2">
          <img src="https://img.freepik.com/free-vector/bookcrossing-library-composition_1284-21973.jpg" alt="" className="w-full h-full object-cover rounded-2xl" />
        </div>
      </div>
    </div>
  );
